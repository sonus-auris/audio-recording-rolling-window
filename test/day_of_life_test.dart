...
void main() {
  ...
  test('overlapping music + speech at midday reads as a jam session', () {
    final notes = summarizer.summarize(detections: [
      det(AcousticDetectionKind.music, DateTime(2026, 6, 11, 13, 0),
          const Duration(minutes: 25)),
      det(AcousticDetectionKind.speech, DateTime(2026, 6, 11, 13, 5),
  ...
  test('snoring overnight reads as sleep', () {
    final notes = summarizer.summarize(detections: [
      det(AcousticDetectionKind.snore, DateTime(2026, 6, 11, 2, 0),
  ...
  test('a fast GPS run becomes a Drive note', () {
    ...
    final notes = summarizer.summarize(detections: const [], geo: [
  ...
  test('day title and description format the way SoundCloud will show them', () {
    final day = DayOfLife(
