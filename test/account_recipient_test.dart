...
void main() {
  ...
  test('account seal/open round-trips a DEK', () async {
    ...
    final dek = sample(32);
    final blob =
        await recipient.seal(publicKey: account.publicKey, dekBytes: dek);
    final recovered =
        await recipient.open(privateSeed: account.privateSeed, blob: blob);
  ...
  test('a different account key cannot open the sealed DEK', () async {
    ...
    final blob =
        await recipient.seal(publicKey: account.publicKey, dekBytes: sample(32));
    expect(
  ...
  test('v2 container: phone reads its own, desktop (account key) reads it too',
      () async {
    ...
    final viaPhone =
        await cipher.open(container: container, unwrapDek: phone.unwrapDek);
    ...
    final viaDesktop = await aead.decrypt(box, secretKey: SecretKey(dekBytes));
    expect(viaDesktop, equals(plaintext));
  });

  test('a second phone (no account key) cannot read another device segment',
      () async {
  ...
  test('v1 containers (no account key) still round-trip', () async {
    ...
    final container =
        await cipher.seal(plaintext: plaintext, wrapDek: phone.wrapDek);
