...
void main() {
  ...
  test('writes a spectral track whose dominant bin tracks a 1 kHz tone', () async {
    const rate = 16000;
    const freq = 1000;
    ...
    final sidecar = await SpectralSidecar(fftSize: 1024)
        .writeForWav(wav.path, sampleRate: rate, channels: 1);
    expect(sidecar, isNotNull);
    expect(sidecar!.path, endsWith('.features.json'));

    final json = jsonDecode(await sidecar.readAsString()) as Map<String, dynamic>;
    ...
    final dominant = frames.map((f) => (f as List)[5] as num).toList()..sort();
    final median = dominant[dominant.length ~/ 2];
    // Bin width is 16000/1024 ≈ 15.6 Hz; the dominant bin should sit on ~1 kHz.
    expect((median - 1000).abs() < 60, isTrue, reason: 'median dominantHz=$median');
